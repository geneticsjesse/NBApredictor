...
def read_train():
    ...
    df = pd.read_csv('./scaled_training_sets/training2015-2021_outliers_removed_scaled.csv', header=0)
...
def read_test():
    # load the dataset; header is first row
    df = pd.read_csv('./scaled_training_sets/test_set_outliers_removed_scaled.csv', header=0)
...
fig1.savefig(f"./hyperparameterOptimization/modelComparison_beforeOptimization_baseline.png")
plt.close(fig1)
#plt.show()
...
fig2.savefig(f"./hyperparameterOptimization/modelComparison_afterOptimization_baseline.png")
plt.close(fig2)
#plt.show()
...
for name, model in optimized_models:
    ...
    plt.savefig(f'confusionMatrices/{name}_ConfusionMatrix_baseline.png')